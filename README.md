...
源码目录可直接双击 `启动美股量化研究台.cmd`。默认界面采用多级工作台：
左侧一级菜单为“今日运行、针对性验证·内部影子、研究工坊、系统与审计”，
内容区二级页签拆分具体功能，策略和订单详情保留三级页签。切换菜单只改变显示页，
不会重建控件或销毁运行状态；滚动仅发生在当前功能页。排障时可临时设置
`US_QUANT_LEGACY_UI=1` 回到旧多标签界面。
...
2. 不要把 Secret 发到聊天中。
3. 打开“系统与审计 → 系统设置”，选择深色或浅色主题，并设置默认行情源。
...
需要针对某只股票或 ETF 做日内 T 时，在“针对性验证 · 内部影子”
...
桌面默认采用多级流程工作台：“今日运行”完成账户刷新、候选准备、行情检查、
Paper 启动、暂停、停止和人工对账；“针对性验证 · 内部影子”只运行本地仿真；
“研究工坊”组织标的池、历史数据、扫描、策略版本和回测证据；“系统与审计”
收纳事件、风险边界、连接参数和凭据。旧多标签界面只作为环境变量回退入口。
行情数据源与券商连接是两种独立角色：Finnhub/Alpaca/IBKR 可提供行情，
IBKR Gateway Paper 负责账户、持仓和模拟订单连接。

“今日运行 → Paper 自动量化”是多标的主工作区：从最新广域扫描动态选取合格候选，
...
“研究工坊 → 回测工作区”可按不可变策略版本运行买入持有、双均线趋势、
