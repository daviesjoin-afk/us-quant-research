...
源码目录可直接双击 `启动美股量化研究台.cmd`。界面采用 Desktop UI v2：
左侧一级导航固定为“总览、行情、账户、策略、风控、订单与成交、研究、系统”，
“研究”和“系统”用二级页签拆分具体功能，自动量化详情与针对性验证保留三级页签。
切换导航只改变显示页，不会重建控件或销毁运行状态。架构说明见
[docs/TRADING_ARCHITECTURE_V2.md](docs/TRADING_ARCHITECTURE_V2.md)。
...
外部行情源默认通过本机 Clash 兼容代理（`http://127.0.0.1:7897`）连接，
避免受限网络直连被中断；设置 `HTTPS_PROXY` 环境变量可覆盖默认代理。
Clash 未运行时自动回退直连并提示。代理仅用于外部行情 WebSocket
（Finnhub/Alpaca），IBKR 始终走本机 Gateway 直连，不受影响。

1. 使用 Finnhub API Key，或在 Alpaca 创建免费 Market Data API Key。
2. 不要把 Secret 发到聊天中。
3. 打开“系统 → 系统设置”，选择深色或浅色主题，并设置默认行情源。
...
Finnhub 行情新鲜度按美股时段自适应：常规盘要求成交打印不超过 20 秒，
盘前、盘后和隔夜使用不超过 120 秒的有界窗口。常规盘启动至少需要 3 个
fresh 候选；扩展时段因成交稀疏至少需要 1 个。过期报价仍禁止进入 Paper
下单，WebSocket 已认证但暂时没有新成交时会明确显示“等待新的成交打印”。
扩展时段的 Paper 候选优先按 20 日平均成交额排序，以提高盘前、盘后获得
真实成交打印的概率；常规盘仍按策略信号和评分排序。SPY、QQQ 等市场参考
只用于市场状态门控，不占用候选名额，也不会被计入“可下单候选 fresh”。
...
需要针对某只股票或 ETF 做日内 T 时，在“研究 → 针对性验证”
...
桌面采用 Desktop UI v2 的八个一级入口：“订单与成交”承载 Paper 启动、暂停、
停止和人工对账；“研究 → 针对性验证”只运行本地仿真；“策略”管理不可变策略
版本的参数、生命周期与晋级门；“研究”组织标的池、历史数据、扫描和回测证据；
“系统”收纳运行事件、风险边界、连接参数和凭据。行情数据源与券商连接是两种
独立角色：Finnhub/Alpaca/IBKR 可提供行情，IBKR Gateway Paper 负责账户、
持仓和模拟订单连接。

“订单与成交 → Paper 自动量化”是多标的主工作区：从最新广域扫描动态选取合格候选，
...
Paper 与内部影子共享互斥执行租约。Paper 只有在本地持仓、挂单、券商持仓和
未对账记录全部清零后才算 finalized 并释放租约；`HALTED` 不会自动重连、
重挂订单或恢复信号，必须人工读取券商状态、人工对账并再次显式确认。程序关闭
同样受 finalized 状态约束，不能跳过未清持仓、订单或对账记录。

“研究 → 回测”可按不可变策略版本运行买入持有、双均线趋势、
...
```powershell
.\scripts\install_ibkr_api.ps1 -PythonClientPath "C:\TWS API\source\pythonclient"
```
...
本地视觉验收截图（`research/artifacts/desktop_*.png`，由
`scripts/render_desktop_preview.py` 生成）不随仓库分发：运行页会打印本机
绝对状态目录，截图因此携带本机路径。
