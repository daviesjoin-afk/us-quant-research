...
| **Paper** | `PaperWorkflowController` (phase / active plan / execution lease); `PaperTradingService` (candidate and active broker connections) | `PaperOrchestrator` (launch half: `session_published` → `_on_paper_session_published`); `MainWindow` still renders the active session | `PaperTradingService`, the injected session-build seam (`_build_paper_session`), the workflow controller | `start`, signals (`refused`, `log_requested`, `session_published`, `runtime_event_requested`) | Launch only: `ExecutionPage.start_requested` → `_confirm_and_start_auto_quant` (the confirmation dialog, which may not move into the capability) → `PaperOrchestrator.start`. The active-session bridges — polling, stream ingress, pause/resume, stop, reconciliation, finalization — are still the window's | Paper — v2O-E1 launch orchestration complete / v2O-E partial |
