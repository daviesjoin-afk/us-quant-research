...
| **Paper** | `PaperWorkflowController` (phase / active plan / execution lease / latest result); `PaperTradingService` (candidate and active broker connections) | `PaperOrchestrator` (the whole run: launch *and* the active session); `MainWindow` renders the published result | `PaperTradingService`, the injected session-build seam (`_build_paper_session`), a market-snapshot provider, the workflow controller | `start`, `on_market_snapshot`, `poll`, `pause`, `resume`, `stop`, `result`, `runtime_active`, `has_runtime_obligations`, signals (`refused`, `log_requested`, `result_changed`, `runtime_event_requested`) | `start_requested` → `_confirm_and_start_auto_quant` (the confirmation dialog, which may not move into the capability) → `start`. `pause_requested` / `resume_requested` / `stop_requested` and `paper_order_timer.timeout` reach the capability **directly**. Market `snapshot_changed` → `_on_market_snapshot_changed` hands the fact over → `on_market_snapshot` (the fan-out decides nothing about Paper). `result_changed` → `_on_paper_result_changed`: render, plus a temporary E3 bridge. The market stop/switch interlock reads `has_runtime_obligations` and Shadow's capital gate reads `runtime_active`, both from the canonical result rather than a window cache. Still the window's: reconciliation, HALT recovery, finalization, `closeEvent`, and execution render ownership | Paper — v2O-E1 launch orchestration complete / v2O-E2 active runtime orchestration complete / v2O-E partial |
