...
class AutoQuantEngine:
    ...
    def on_stream(
        ...
        *,
        ...
        reference_ready = {
            ...
            for quote in snapshot.quotes
            if (
                quote.symbol in self._reference_histories
                and quote.realtime_ready
            )
        }
        ...
        for symbol, quote in reference_ready.items():
            assert quote.bid is not None and quote.ask is not None
            self._update_reference_minute(
                symbol, now, (quote.bid + quote.ask) / Decimal("2")
            )
        ...
        if self._stop_requested:
            if not self.positions and not self.pending:
                ...
                # CR-1 regression guard: never re-submit a SELL for a symbol
                # that already has an active sell intent.  The old code
    ...
    def _evaluate_entry(
        ...
        if self.estimated_realized_pnl <= -daily_loss_limit:
            ...
            return
        regime_block = self._entry_regime_block(reference_ready)
    ...
    def _check_exit(
        ...
        for position in (self.positions.get(quote.symbol),):
            ...
            if reason is not None:
                # H-11 fix: price risk exits off the real bid, never the
                # mid-mark; a limit derived from mid can rest above the bid
                # and never fill, forcing the 90-second intervention halt.
                self._emit_exit(
    ...
    def _estimated_equity(self) -> Decimal:
        ...
        for position in self.positions.values():
            ...
            position_value += mark * position.quantity
        return self.estimated_cash + position_value
    ...
    def _entry_regime_block(
        ...
        for symbol in self.market_reference_symbols:
            ...
            one_minute_return = history[-1][1] / history[-2][1] - Decimal("1")
