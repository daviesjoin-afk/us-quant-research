...
from us_quant.research_manifest import build_research_run_manifest
...
class ResearchMode(str, Enum):
    """The candidate-selection policy for an exploratory research run."""

    FIXED_DAILY = "fixed_daily"
    EXPLORATORY_GRID = "exploratory_grid"


FIXED_DAILY_CANDIDATE = CrossSectionalCandidate(
    lookback_days=126,
    rebalance_days=21,
    top_n=3,
)
...
def resolve_research_candidates(
    mode: ResearchMode | str,
    *,
    exploratory_candidates: tuple[
        CrossSectionalCandidate, ...
    ] = DEFAULT_CANDIDATES,
) -> tuple[CrossSectionalCandidate, ...]:
    """Return the declared candidate catalog without selecting a winner."""
    selected_mode = ResearchMode(mode)
    if selected_mode is ResearchMode.FIXED_DAILY:
        return (FIXED_DAILY_CANDIDATE,)
    if not exploratory_candidates:
        raise ValueError("exploratory candidate grid must not be empty")
    return tuple(exploratory_candidates)
...
def run_cross_sectional_research(
    ...
    *,
    ...
    mode: ResearchMode | str = ResearchMode.FIXED_DAILY,
    candidates: tuple[CrossSectionalCandidate, ...] | None = None,
    minimum_train_days: int = 504,
    test_days: int = 126,
) -> dict:
    selected_mode = ResearchMode(mode)
    if candidates is not None and selected_mode is ResearchMode.FIXED_DAILY:
        raise ValueError(
            "fixed_daily research always evaluates its declared candidate; "
            "use exploratory_grid for a candidate grid"
        )
    candidate_catalog = resolve_research_candidates(
        selected_mode,
        exploratory_candidates=(
            DEFAULT_CANDIDATES if candidates is None else candidates
        ),
    )
    data, records, loaded_series = _load_data(
        universe,
    ...
    return {
        ...
        "deployable": False,
        "research_mode": selected_mode.value,
        "candidate_catalog": [
            _candidate_payload(candidate)
            for candidate in candidate_catalog
        ],
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "run_manifest": build_research_run_manifest(
            universe,
            records,
            loaded_series,
        ).to_dict(),
        ...
        "methodology": {
            ...
            "candidates": [row.name for row in candidate_catalog],
            "costs": {
                "per_share_commission": float(
                    config.execution.per_share_commission
                ),
...
def _load_data(
    ...
    *,
    ...
    return data, records, loaded_series
