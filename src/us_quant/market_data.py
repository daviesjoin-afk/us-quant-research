...
class ResearchDataMode(str, Enum):
    """The intended use of a loaded historical series.

    ``EXPLORATORY`` deliberately retains support for the existing research
    archive.  ``EXECUTION_CANDIDATE_PIT_RAW`` is a stricter, opt-in gate for
    future callers that wish to promote research results towards Paper
    execution: it accepts only trade-price OHLC data accompanied by explicit
    point-in-time universe membership evidence.
    """

    EXPLORATORY = "exploratory"
    EXECUTION_CANDIDATE_PIT_RAW = "execution_candidate_pit_raw"


class ResearchDataEligibilityError(ValueError):
    """Raised when data is not eligible for its requested research mode."""


@dataclass(frozen=True, slots=True)
class ResearchDataEligibility:
    mode: ResearchDataMode
    eligible: bool
    reasons: tuple[str, ...]


def evaluate_research_data_eligibility(
    series: LoadedDailySeries,
    *,
    mode: ResearchDataMode,
) -> ResearchDataEligibility:
    """Return an immutable, machine-readable eligibility decision.

    The exploratory mode intentionally does not retroactively reject legacy
    artifacts.  Execution-candidate mode is intentionally conservative: a
    missing metadata field is not evidence, so legacy/adjusted datasets and
    current-only universe snapshots are rejected.
    """

    if mode is ResearchDataMode.EXPLORATORY:
        return ResearchDataEligibility(mode=mode, eligible=True, reasons=())

    reasons: list[str] = []
    if series.price_basis != "raw_trade_ohlc":
        reasons.append(
            "execution-candidate research requires price_basis=raw_trade_ohlc"
        )
    if not series.point_in_time_membership:
        reasons.append(
            "execution-candidate research requires point-in-time universe membership"
        )
    return ResearchDataEligibility(
        mode=mode,
...
def require_research_data_eligibility(
    series: LoadedDailySeries,
    *,
    mode: ResearchDataMode,
) -> ResearchDataEligibility:
    """Return a passing eligibility decision or raise a clear gate error."""

    eligibility = evaluate_research_data_eligibility(series, mode=mode)
    if not eligibility.eligible:
        raise ResearchDataEligibilityError(
...
def save_historical_series(
    ...
    *,
    ...
    # CR-5 fix: self-hash the normalized payload so readers can verify the
    # file was not edited in place.  ``source_sha256`` only fingerprints the
    # raw file, so without this field a modified normalized file would load
    # silently.  Old files without the field remain readable (legacy mode).
    normalized_canonical = json.dumps(
        normalized_payload,
        ensure_ascii=False,
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
    normalized_payload["normalized_sha256"] = sha256(
...
def load_latest_normalized_series(
    ...
    *,
    ...
    research_mode: ResearchDataMode = ResearchDataMode.EXPLORATORY,
) -> LoadedDailySeries:
    ...
    # CR-5 fix: verify the normalized file was not edited in place when the
    # self-hash field is present (files written before this fix have no field
    # and are accepted as legacy data).
    stored_normalized_hash = payload.pop("normalized_sha256", None)
    if stored_normalized_hash is not None:
        canonical = json.dumps(
            payload,
            ensure_ascii=False,
            sort_keys=True,
        ...
        if sha256(canonical).hexdigest() != stored_normalized_hash:
            raise ValueError(
                f"normalized data integrity check failed: {path}"
            )
    ...
    loaded = LoadedDailySeries(
        ...
        point_in_time_membership=(
            payload.get("point_in_time_membership") is True
        ),
    )
    require_research_data_eligibility(loaded, mode=research_mode)
    return loaded
