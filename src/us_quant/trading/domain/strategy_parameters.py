...
def validate_strategy_parameters(
    ...
    if strategy_id == "dual-ma-trend":
        ...
        if strategy_id == "intraday-auto-rotation":
            references = normalized.setdefault(
                "market_reference_symbols", []
            )
            if not isinstance(references, list):
                raise StrategyParameterError(
                    "market_reference_symbols must be a symbol list"
                )
            normalized_references = []
            for symbol in references:
                if not isinstance(symbol, str) or not symbol.strip():
                    raise StrategyParameterError(
                        "market_reference_symbols contains an invalid symbol"
                    )
                normalized_references.append(symbol.strip().upper())
            if len(set(normalized_references)) != len(normalized_references):
                raise StrategyParameterError(
                    "market_reference_symbols must not contain duplicates"
                )
            normalized["market_reference_symbols"] = normalized_references
