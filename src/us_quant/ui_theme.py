...
def build_stylesheet(palette: ThemePalette) -> str:
    ...
    return f"""
        ...
        #workflowAnchor:hover {{
            background: {p.accent_surface};
            color: {p.accent};
        }}
        #workflowAnchor[active="true"] {{
            background: {p.accent_surface};
            color: {p.accent};
            border-left: 3px solid {p.accent};
            font-weight: 700;
        }}
        #workflowAnchor[compact="true"] {{
            font-size: 20px;
            padding: 7px 0;
            text-align: center;
        }}
        #workflowScrollArea, #workflowScrollArea > QWidget > QWidget {{
            background: transparent;
        }}
        #workflowScrollContent {{ background: transparent; }}
        #workflowModuleStack {{ background: transparent; }}
        #workflowModulePage {{
            background: {p.panel};
            border: 1px solid {p.border};
            border-radius: 9px;
        }}
        #workflowModuleHeading {{
            color: {p.heading};
            background: {p.panel};
        ...
        #workflowPageScroll,
        #workflowPageScroll > QWidget > QWidget {{
            background: transparent;
            border: none;
        }}
        #workflowSecondaryTabs::pane {{
        ...
        #workflowSectionHeader:hover {{ background: {p.surface_alt}; }}
