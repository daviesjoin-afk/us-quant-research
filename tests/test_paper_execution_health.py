...
class PaperExecutionHealthTests(unittest.TestCase):
    ...
    def test_exact_unreconciled_summary_is_not_hidden_by_capped_detail_rows(self) -> None:
        """The health verdict uses the uncapped aggregate, not row details."""
        ...
        self.assertEqual(result.unreconciled_order_count, 1)
        self.assertIn("orders_in_flight", {issue.code for issue in result.issues})

    def test_exact_terminal_summary_halts_even_when_details_are_capped(self) -> None:
        result = evaluate_paper_execution_health(
            connection=_connection(),
            broker_state=_broker_state(quantity=Decimal("0")),
            engine_snapshot=_snapshot(quantity=0),
