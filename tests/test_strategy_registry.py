...
class StrategyRegistryTests(unittest.TestCase):
    ...
    def test_seeded_regime_gate_version_remains_research_only(self) -> None:
        with TemporaryDirectory() as directory:
            registry = StrategyRegistry(
                Path(directory) / "strategies.sqlite3"
            )
            ...
            record = next(
                row
                for row in registry.list_records()
                if (
                    row.strategy_id == "intraday-auto-rotation"
                    and row.semver == "1.2.0-research"
                )
            )
        self.assertEqual(record.status, "research")
        self.assertFalse(record.gate_passed)
        self.assertEqual(
            record.parameters["market_reference_symbols"], ["SPY", "QQQ"]
        )
